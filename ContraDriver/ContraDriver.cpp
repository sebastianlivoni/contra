...
#include "ContraDriver.h"

struct ContraDriver_IVars
{
    OSArray *elements;
...
bool ContraDriver::init()
{
    if (!super::init()) {
        return false;
    }
    
    ivars = IONewZero(ContraDriver_IVars, 1);
    if (!ivars) {
        return false;
    }
...
void ContraDriver::free()
{
    ...
    IOSafeDeleteNULL(ivars, ContraDriver_IVars, 1);
    super::free();
}

kern_return_t
IMPL(ContraDriver, Start)
{
    kern_return_t ret;
...
bool ContraDriver::parseElements(OSArray *elements)
{
...
void ContraDriver::handleScrollReport(uint64_t timestamp, uint32_t reportID) {
